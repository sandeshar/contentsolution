...
export async function POST(request: Request) {
    try {
        // Always clean existing services data before seeding
        ...
        try {
            ...
            // swallow errors to keep seeder resilient
        }
        ...
        // find an explicit published status (case-insensitive name match), fallback to first status
        ...
        // Avoid creating duplicate category entries if one already exists
        const [existingCat] = await db.select().from(serviceCategories).where(eq(serviceCategories.slug, categorySlug)).limit(1);
        if (!existingCat) {
            ...
            for (const [index, s] of serviceData.entries()) {
                // insert detail only when missing (idempotent)
                const [existingDetail] = await db
                    .select()
                    .from(servicesPageDetails)
                    .where(eq(servicesPageDetails.key, s.key))
                    .limit(1);
                if (!existingDetail) {
                    ...
                    `,
                        ...
                        for (const [vIndex, variant] of postVariants.entries()) {
                            ...
                            // ensure we don't insert duplicate post slug
                            const [existingPost] = await db.select().from(servicePosts).where(eq(servicePosts.slug, `${s.key}-${variant.suffix}`)).limit(1);
                            if (!existingPost) {
                            ...
                            const basePostContent = contentMap[s.key]
                                ...
                                : generateLongContent(s.title, 28);
                            // Insert base post only if missing
