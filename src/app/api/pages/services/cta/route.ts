...
export async function GET(request: NextRequest) {
    try {
        ...
        if (id) {
            const cta = await db.select().from(servicesPageCTA).where(eq(servicesPageCTA.id, parseInt(id))).limit(1);
        ...
        const cta = await db.select().from(servicesPageCTA).where(eq(servicesPageCTA.is_active, 1)).limit(1);
